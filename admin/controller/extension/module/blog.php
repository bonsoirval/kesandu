...
class ControllerExtensionModuleBlog extends Controller {
	...
	public function index() {
		...
		//confirm request is by post and validate input data
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			//fetch success message set in blog language file
			$this->session->data['success'] = $this->language->get('text_success');

			//redirect user to marketplace/extension and pass user_token = session variable
			$this->response->redirect($this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true));
		}

		//set warning if one exists
		...
		//declare breadcrumbs array
		$data['breadcrumbs'] = array();

		//set home text and it url in breadcrumbs
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_home'),
			'href' => $this->url->link('common/dashboard', 'user_token=' . $this->session->data['user_token'], true)
		);

		//set extension text and it url in breadcrumbs
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_extension'),
			'href' => $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true)
		);

		//set page title and the url on breadcrumbs
		//if the module (its id) is not set
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
